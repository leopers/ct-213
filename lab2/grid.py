...
class CostMap(object):
    ...
    def get_edge_cost(self, start, end):
        ...
        return (
            factor
            * (
                self.get_cell_cost(start[0], start[1])
                + self.get_cell_cost(end[0], end[1])
            )
            / 2.0
        )
    ...
    def add_obstacle(self, rectangle):
        ...
        """
        self.add_rectangle(
            (rectangle[0] - 1, rectangle[1] - 1, rectangle[2] + 2, rectangle[3] + 2),
            2.0,
        )
...
class NodeGrid(object):
    ...
    def get_successors(self, i, j):
        ...
        for di in range(-1, 2):
            for dj in range(-1, 2):
                if di != 0 or dj != 0:
                    if self.cost_map.is_index_valid(
                        i + di, j + dj
                    ) and not self.cost_map.is_occupied(i + di, j + dj):
